...
from imaging.analysis import getPoints
...
def stitchImages(img1_cp, img2, sepMag,
                padNaN=False, vmax=None, vmin=None):
    """Allows user to pick fiducials for both images.
    Function then computes the transform to move img2
    to img1 reference frame.
    Updated
    """
    img1 = np.copy(img1_cp)
    #Get both fiducials
    xf1,yf1 = getPoints(img1, vmax, vmin)
    xf2,yf2 = getPoints(img2, vmax, vmin)
    ...
    if sepMag:
        x2, y2 = transformCoords_wSeparateMag(x2,y2,tx,ty,theta,x_mag,y_mag)
    else:
        x2,y2 = transformCoords(x2,y2,tx,ty,theta)
    ...
    find = np.logical_and(np.sqrt((y1-xf1[0])**2+(x1-yf1[0])**2) < 15.,\
                       np.isnan(img1).flatten())
    for i in range(1,np.size(xf1)):
        find = np.logical_or(find,\
                np.logical_and(np.sqrt((y1-xf1[i])**2+(x1-yf1[i])**2) < 15.,\
                        np.isnan(img1).flatten()))

    #Include newimg data
    img1[np.isnan(img1)] = newimg[np.isnan(img1)]
    #Reset fiducials to NaNs
    img1[find.reshape(np.shape(img1))] = np.nan

    #Return translations to automatically pad next image
    return img1, newimg, tx, ty, theta, x_mag, y_mag
